...
function initTwitterOauth() {
    console.log("here");
    oa = new OAuth(
        "https://twitter.com/oauth/request_token", "https://twitter.com/oauth/access_token", config.consumerKey, config.consumerSecret, "1.0A", "http://127.0.0.1:3000/twitter/authn/callback", "HMAC-SHA1"
    );
}
...
function makeTweet(cb) {
    ...
    oa.post(
        "https://api.twitter.com/1.1/statuses/update.json", user.token, user.tokenSecret, {
...
function makeDm(sn, cb) {
    oa.post(
        "https://api.twitter.com/1.1/direct_messages/new.json", user.token, user.tokenSecret, {
            "screen_name": sn,
...
app.get(
    '/twitter/authn/callback', passport.authenticate(
        'twitterAuthn', {
...
app.get(
    '/twitter/authz/callback', passport.authenticate(
        'twitterAuthz', {
